...
(async () => {
    try {
        ...
        // const apiFiles = await _readdir(`./app/routes/${getAdminFolderName}`);
        ...
        console.log(`Site is running on ${(global.BASE_URL && global.BASE_URL !== '') ? global.BASE_URL : `http://${process.env.HOST}:${getPort}`}`);
    } catch (error) {
        console.error(error);
    }
})();
